...
def display_metrics(deals_df: pd.DataFrame, shifts_df: pd.DataFrame, selected_channels: list = None):
    ...
    value_sum = deals_df['deal_value'].sum()
    ...
    cols = st.columns(3)
    cols[0].metric('💰 میزان فروش', f'{value_sum:,.0f} ریال')
...
def display_reward_section(deals_for_reward: pd.DataFrame, parameters: dict, user_filter: str = None):
    ...
    # have reached at least 95% of the target.
    # The progress bar is capped at 100%.
    if target > 0:
        percent_of_target = min((current_value / (0.95 * target)) * 100, 100)
    else:
        percent_of_target = 0
    
    # Determine which reward percentage to use (normal vs. growth)
    reward_percent = parameters.get('grow_percent', 0) if target > 0 and current_value >= 0.95 * target else parameters.get('normal_percent', 0)
    st.metric('🎯 تارگت فروش ماه (بر اساس تاریخ خروج)', value=f'{target:,.0f} تومان')
    ...
    if selected_member:
        ...
        cols = st.columns(2)
        cols[0].metric(f'میزان فروش {selected_member}', value=f'{member_value:,.0f} ریال')
        cols[1].metric(f'💰 میزان پاداش {selected_member}', value=f'{member_reward:,.0f} ریال')
...
def social():
    ...
    # For general stats, we use the deal_created_date
...
def render_dashboard(deals_data: pd.DataFrame, shift_data: pd.DataFrame, parameters: dict, user_filter: str = None):
    ...
    """
    month_choice = st.selectbox('ماه مورد نظر را انتخاب کنید:', ['این ماه', 'ماه پیش'])
    ...
    monthly_deals = deals_data[deals_data['jalali_year_month'] == target_month]
    ...
    plot_daily_trend(
        df=monthly_deals.groupby('deal_created_date').size().reset_index(name='تعداد'),
        date_col='deal_created_date',
        value_col='تعداد',
        title='تعداد معاملات روزانه',
        labels={'deal_created_date': 'تاریخ', 'تعداد': 'تعداد معامله'}
    )
    ...
    if not user_filter:
        ...
        seller_values = cols[1].multiselect('انتخاب فروشنده:', options=sellers, default=sellers)
        ...
        if not seller_values or not channel_values:
            ...
            plot_daily_trend(
                df=filtered_deals.groupby('deal_created_date').size().reset_index(name='تعداد'),
                date_col='deal_created_date', value_col='تعداد', title='تعداد معاملات روزانه (فیلتر شده)',
                labels={'deal_created_date': 'تاریخ', 'تعداد': 'تعداد معامله'}
            )
            ...
            if lead_dfs:
                ...
                plot_daily_trend(
                    df=daily_lead_count, date_col='تاریخ میلادی', value_col='تعداد', title='تعداد لیدهای روزانه (فیلتر شده)',
                    labels={'تاریخ میلادی': 'تاریخ', 'تعداد': 'تعداد لید'}
                )

        st.subheader(f'شیفت های {', '.join(str(i) for i in seller_values)}')
        ...
        st.subheader('شیفت های شما')
        st.write(filtered_shifts)
        st.divider()

    
    # Display the reward section for the chosen month's CHECKOUT dates
    deals_for_reward = deals_data[deals_data['checkout_jalali_year_month'] == target_month]
