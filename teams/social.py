...
def safe_to_jalali(gregorian_date):
    ...
    try:
        ...
        logger.error(f"safe_to_jalali({gregorian_date}): {e}")
        return None
...
def get_month_filter_string(month_choice: str) -> str:
    ...
    try:
        ...
        logger.error(f"Error in get_month_filter_string('{month_choice}'): {e}")
        return ""
...
def display_metrics(deals_df: pd.DataFrame, shifts_df: pd.DataFrame, selected_channels: list = None):
    ...
    try:
        ...
        logger.error(f"display_metrics error: {e}")
...
def plot_daily_trend(df: pd.DataFrame, date_col: str, value_col: str, title: str, labels: dict):
    ...
    try:
        ...
        logger.error(f"plot_daily_trend error: {e}")
...
def display_reward_section(deals_for_reward: pd.DataFrame, parameters: dict, user_filter: str = None):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Error generating checkout_jalali_str: {e}")
        ...
        try:
            ...
            logger.error(f"Error drawing reward progress pie: {e}")
        ...
        if not deals_for_reward.empty and user_filter is None:
            try:
                ...
                logger.error(f"display_reward_section team member reward table error: {e}")
        ...
        if selected_member:
            try:
                ...
                logger.error(f"display_reward_section individual member error: {selected_member} - {e}")

    except Exception as e:
        logger.error(f"display_reward_section: {e}")
...
def social():
    ...
    try:
        ...
        if not all(key in st.session_state for key in ['username', 'role', 'data', 'auth']):
            ...
            logger.error("User not authenticated or missing session state keys")
            st.stop()
        ...
        try:
            ...
            logger.error(f"Error loading Social team parameters: {e}")
        ...
        try:
            ...
            logger.error(f"Error loading Social shift sheet: {e}")
        ...
        logger.error(f"social() main error: {e}")
        ...
        if not user_filter:
            ...
            if not seller_values or not channel_values:
                ...
                if lead_dfs:
                    try:
                        ...
                        logger.error(f"Error plotting daily_lead_count: {e}")
            if not filtered_shifts.empty:
                try:
                    ...
                    logger.error(f"Error displaying filtered_shifts: {e}")
            ...
            if not channel_values:
                ...
                if lead_dfs:
                    try:
                        ...
                        logger.error(f"Error plotting user daily_lead_count: {e}")
        ...
        try:
            ...
            if st.form_submit_button('ذخیره تغییرات'):
                ...
                try:
                    ...
                    logger.error(f"Error writing params to sheet: {e}")
                if success:
                    st.success("پارامترها با موفقیت به‌روزرسانی شد.")
                    st.rerun()
                else:
                    logger.error("Failed to update team parameters!")
                    st.error("خطا در هنگام به‌روزرسانی پارامترها!")
        except Exception as e:
            logger.error(f"render_settings_tab error: {e}")